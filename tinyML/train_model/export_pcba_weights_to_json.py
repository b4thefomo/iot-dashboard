#!/usr/bin/env python3
"""
export_pcba_weights_to_json.py — Export trained PCBA Kernel MLP weights to JSON
...
Usage:
    python export_pcba_weights_to_json.py --model pcba_kernel_model/pcba_classifier.keras
"""
...
def main():
    parser = argparse.ArgumentParser(description="Export PCBA Kernel MLP weights to JSON")
    parser.add_argument(
        "--model", type=str,
        default="pcba_kernel_model/pcba_classifier.keras",
        help="Path to trained Keras model"
    )
    parser.add_argument(
        "--params", type=str,
        default="pcba_kernel_model/pcba_deploy_params.json",
    ...
    parser.add_argument(
        "--output", type=str,
        default="pcba_kernel_model/pcba_weights.json",
        help="Output JSON weights file"
    )
    parser.add_argument(
        "--server-output", type=str,
        default="kernel/pcba_mlp_weights.json",
    ...
    test_X = df[PCBA_FEATURE_NAMES].values[test_indices].astype(np.float32)
    ...
    match_rate = matches / len(test_X)
    print(f"  Prediction match rate: {match_rate * 100:.1f}% ({matches}/{len(test_X)})")
    print(f"  Max probability difference: {max_diff:.6f}")

    if match_rate < 0.98:
        print("ERROR: JSON inference does not match Keras predictions. Aborting weight export.")
        sys.exit(1)

    print("  PASS: JSON inference matches Keras")

    # Copy to server directory once validation passes
